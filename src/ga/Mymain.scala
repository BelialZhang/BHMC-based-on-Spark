...
object Mymain {
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setMaster("local[1]").setAppName("bhmc").set("spark.defalut.parallelism","24")//创建环境变量、设置本地化处理、设置名称
    ...
    {
      fileName = "./result//atom"+atomN+"//"+atomN+"Co"+Corate+"_1.txt"
    }
   ...
   // var testpop = Initialization.initialChromosome(atomN,rate)
    ...
    //新版下行原来注释
    populationRDD = populationRDD.partitionBy(new ModPartitioner(numPartitions)).persist()
